...
# 결정경계가 어떤 값을 가지고 분류하는가?
# 양쪽의 확률이 50%가 되는 1.6cm 근방에서 결정경계가 만들어짐
# 분류기는 1.6cm보다 크면 Iris virsinica로 분류, 작으면 아니라고 예측
x_new = np.linspace(0, 3, 1000).reshape(-1, 1)
y_proba = log_reg.predict_proba(x_new)
decision_boundary = x_new[y_proba[:, 1] >= 0.5][0]
...
# 예상 1.6 기준 1.8 -> true, 1.48 -> false
print(f'우리가 원하는 분류가 되는가? 분류기준 {decision_boundary} result --> {test_data}')